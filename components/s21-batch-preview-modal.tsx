...
import { S21BatchPrintLayout } from '@/components/s21-batch-print-layout';
...
export function S21BatchPreviewModal({ items, serviceYear, title, onClose }: Props) {
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      const pages = layoutRef.current.querySelectorAll('[data-s21-page]');
  ...
  const handlePrint = () => {
    ...
    const pages = layoutRef.current.querySelectorAll('[data-s21-page]');
    const bgImgSrcs: string[] = [];
    const overlayHtmls: string[] = [];

    pages.forEach((pageEl) => {
      const canvas = pageEl.querySelector('canvas');
      bgImgSrcs.push(canvas ? canvas.toDataURL('image/png') : '');
      const overlays = pageEl.querySelector('[data-s21-overlays]');
      overlayHtmls.push(overlays ? overlays.innerHTML : '');
    });
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-sm flex flex-col">
      <div className="w-full z-10 bg-[#0F172A]/95 backdrop-blur border-b border-[#1E293B] px-6 py-3 flex items-center justify-between flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-[#64748B]">{items.length} publicador(es) • Ano de serviço {serviceYear}</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
      ...
      <div className="flex-1 overflow-y-auto flex flex-col items-center py-8">
        <div ref={layoutRef}>
          <S21BatchPrintLayout items={items} serviceYear={serviceYear} />
        </div>
      </div>
    </div>
  );
